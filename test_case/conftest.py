...
def work_login_init():
    ...
    url = "https://zxbtest.ebchaincn.com:9080/api/zz-auth/oauth/token"
    data = {
        "grant_type": "bid_tenant",
        "scope": "all",
        "tenantId": "677588",
        "userPhone": "17610836609",
        "roleId": "1123598816738675212"
    }
    headers = {"Authorization": "Basic YWRtaW46YWRtaW5fc2VjcmV0"}
    # 请求登录接口

    res = requests.post(url=url, params=data, verify=True, headers=headers)

    # 拿到access_token和refresh_token，组装cookies
    cookies = "bearer " + res.json()["access_token"]
